...
describe('getOrCreateDeviceId', () => {
  ...
  beforeEach(async () => {
    claudeDirectory = await mkdtemp(join(tmpdir(), 'reup-device-id-test-'))
  ...
  it('writes the id file under the reup subdirectory', async () => {
    await getOrCreateDeviceId()
    const stored = await readFile(join(claudeDirectory, 'reup', 'device-id'), 'utf8')
    expect(stored.trim()).toBe(hostname())
  })

  it('reads an existing id file without overwriting it', async () => {
    const customId = 'my-custom-device'
    const reupDirectory = join(claudeDirectory, 'reup')
    await mkdir(reupDirectory, { recursive: true })
    const { writeFile } = await import('node:fs/promises')
    await writeFile(join(reupDirectory, 'device-id'), customId, 'utf8')
