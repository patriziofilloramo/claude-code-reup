...
import { BRAND_COLOR, BRAND_COLOR_DEEP, BRAND_COLOR_MID, SWOOP_PATH } from '../../src/brand.js'
...
function renderBrandMarkup(): string {
  return `<div class="brand"><span class="mark brand-mark"><svg width="72%" height="72%" viewBox="0 0 256 256" fill="none"><path fill="rgba(255,255,255,0.95)" d="${SWOOP_PATH}"/></svg></span><span class="brand-copy"><span class="brand-title">Swoop</span><span class="brand-subtitle">claude code</span></span></div>`
}

const DASHBOARD_CSS = String.raw`
:root{color-scheme:light dark;--bg:var(--vscode-editor-background);--panel:var(--vscode-sideBar-background);--line:var(--vscode-widget-border);--muted:var(--vscode-descriptionForeground);--accent:var(--vscode-focusBorder,var(--vscode-button-background));--good:var(--vscode-testing-iconPassed);--warn:var(--vscode-editorWarning-foreground);--bad:var(--vscode-editorError-foreground)}
...
function rail(){return '<aside class="rail"><div class="section-title">Focus</div>'+nav('all','All sessions',model.summary.sessions-model.summary.archived)+nav('workspace','Current workspace',model.sessions.filter(s=>workspaceIds.has(s.projectId)&&!s.archived).length)+nav('active','Active now',model.summary.active)+nav('attention','Needs attention',model.summary.attention)+nav('archived','Archived',model.summary.archived)+'<div class="section-title">Projects</div>'+model.projects.map(p=>'<button class="project '+(project===p.id?'active':'')+'" data-project="'+attr(p.id)+'"><span class="cloud '+(p.memoryStatus||'')+'">'+(p.memoryStatus&&p.memoryStatus!=='none'?'☁':'□')+'</span><span>'+esc(p.name)+'</span><span class="count">'+p.sessionCount+'</span></button>').join('')+'</aside>'}
...
function previewHtml(p){return section('What you asked for',p.goal,false)+section('Where Claude left off',p.lastResponse,true)+section('Plan',p.automaticContext.plan&&p.automaticContext.plan.text,true)+todos(p)+files('Files touched',p.touchedFiles)+files('Files read',p.automaticContext.readFiles)}
...
function bindActions(){document.querySelectorAll('[data-action]').forEach(x=>x.onclick=e=>{e.stopPropagation();const s=model.sessions.find(v=>v.id===x.dataset.id);if(s)post(x.dataset.action,{projectId:s.projectId,sessionId:s.id,...(x.dataset.target?{target:x.dataset.target}:{})})});document.querySelectorAll('[data-resume-primary]').forEach(x=>x.onclick=e=>{e.stopPropagation();const s=model.sessions.find(v=>v.id===x.dataset.resumePrimary);if(!s)return;if(resumeCapabilities.preferredTarget)post('resume',{projectId:s.projectId,sessionId:s.id,target:resumeCapabilities.preferredTarget});else showResumeMenu(e.clientX,e.clientY,s.id)});document.querySelectorAll('[data-resume-menu]').forEach(x=>x.onclick=e=>{e.stopPropagation();showResumeMenu(e.clientX,e.clientY,x.dataset.resumeMenu)});document.querySelectorAll('[data-file]').forEach(x=>x.onclick=()=>{const s=model.sessions.find(v=>v.id===selected);if(s)post('openFile',{projectId:s.projectId,sessionId:s.id,path:x.dataset.file})})}
