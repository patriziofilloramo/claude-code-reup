...
import { projectMemoryDescription } from './formatting.js'
...
const PROJECT_MEMORY_DESCRIPTIONS = {
  green: projectMemoryDescription('green'),
  grey: projectMemoryDescription('grey'),
  orange: projectMemoryDescription('orange'),
}
...
<script nonce="${nonce}">const BRAND_MARKUP=${JSON.stringify(brandMarkup)};const PROJECT_MEMORY_DESCRIPTIONS=${JSON.stringify(PROJECT_MEMORY_DESCRIPTIONS)};${DASHBOARD_SCRIPT}</script></body></html>`
}
...
function projectCloud(p){const status=p.memoryStatus;if(!status||status==='none')return '<span class="cloud empty" aria-hidden="true">☁</span>';return '<span class="cloud '+attr(status)+'" title="'+attr(PROJECT_MEMORY_DESCRIPTIONS[status]||'')+'">☁</span>'}
