...
const USAGE = `Usage:
  reup config                    Open interactive settings UI
  reup config get [key]          Show one or all values
  reup config set <key> <value>  Save a setting
  reup config reset [key]        Reset one or all settings to defaults
  ...
  switch (subcommand) {
    case undefined: {
      await openConfigInterface({
        ...
        nonInteractiveAlternative: 'use `reup config get`, `set`, or `reset` in scripts',
      })
      return
    }
    case 'get':
...
async function handleSet(key?: string, value?: string): Promise<void> {
  if (!key || value === undefined) {
    failCommand('usage: reup config set <key> <value>')
    return
  }
