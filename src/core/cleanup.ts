...
// Fast, deterministic, scriptable. Run `ccm cleanup --help` for user-facing docs.
//
...
export function summariseCandidates(candidates: CleanupCandidate[]): string {
  ...
  return `${candidates.length} cleanup candidate${candidates.length === 1 ? '' : 's'}: ${parts.join(', ')} — run \`ccm cleanup\` to review`
}
