...
session health — they show you titles and dates; Swoop shows you _state_.
...
| **Lost & Found**              | Automatically surfaces three categories: sessions approaching Claude Code's cleanup window (expiring), sessions whose recorded path no longer exists (path-missing), and transcripts present on disk but absent from any project index (orphaned). Available in the web UI panel and via `swoop doctor`. |
| **swoop doctor**              | Non-destructive, local-only diagnosis command. Checks for stale sidecar locks, broken or absent indices, orphaned transcripts, and missing project paths. Every finding includes an explanation and a suggested action.                                                                                  |
| **Index corruption recovery** | When `sessions-index.json` is absent or corrupt, Swoop walks the transcript directory and reconstructs session metadata from raw JSONL events. Index corruption does not hide sessions — they surface automatically. _This is a gap across all tools reviewed in the 2026 competitive survey._           |

**Unique to Swoop:** all of the above. Branch drift detection, the remote-active
...
| **Ranked shell completion** | Session-ID completion for `swoop resume` and `swoop handoff` across PowerShell, Bash, and Zsh. Completions are ranked: current-project sessions first, then active sessions, then recent activity. No session titles or transcript content are exposed to the shell. |
...
| **Status-line integration**  | Reads the Claude Code terminal status line as a secondary source for model, agent, and live context detail. Never silently replaces an existing status line; `swoop usage setup --replace` is required for that.             |
...
| **Colour-coded thresholds**  | Cyan normally, yellow at 80%, orange at 90%, red at 100%. Applied consistently across TUI, web, and `swoop usage`.                                                                                                           |
...
| **Opt-in, fully reversible** | Usage capture is off by default. `swoop usage setup` enables it; `swoop usage remove` reverses all changes and deletes the local cache. The OAuth token is held in memory only — never logged or written to disk.            |

**Unique to Swoop:** account limits + per-session context in the same always-on
...
| **Search qualifiers**             | All qualifiers available in `swoop list` work in the TUI search bar.                                                                                                    |
...
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `swoop`                               | Open TUI                                                                                                                                            |
| `swoop web`                           | Open browser UI                                                                                                                                     |
| `swoop resume [id]`                   | Interactive global picker, or resume by ID or unambiguous prefix                                                                                    |
| `swoop search <query>`                | Interactive picker with pre-filled search                                                                                                           |
| `swoop search --deep <q>`             | Full-content transcript search with interactive picker                                                                                              |
| `swoop list [query]`                  | Compact human table, globally filtered. `--json` for machine-readable output                                                                        |
| `swoop inbox`                         | Attention-sorted summary of active and at-risk sessions                                                                                             |
| `swoop handoff [session]`             | Compact Markdown continuation packet: last goal, transcript summary, edited files, open todos. Unavailable facts marked explicitly, never inferred. |
| `swoop doctor`                        | Non-destructive local health check                                                                                                                  |
| `swoop usage`                         | Show observed usage and data freshness                                                                                                              |
| `swoop usage setup / remove / toggle` | Manage usage capture integration                                                                                                                    |
| `swoop sync`                          | Open the interactive shared-session-storage panel _(experimental)_                                                                                  |
| `swoop sync link / unlink`            | Manage experimental shared session storage for a project, with an optional explicit path for scripts                                                |
| `swoop config get/set/reset <key>`    | Read and write persistent user preferences                                                                                                          |
...
| `swoop --help / --version`            | Help and version                                                                                                                                    |

`swoop list` filters combine with AND semantics. `--active`, `--attention`,
`--archived`, `--project`, `--status`, `--limit`, `--json` are all composable.

**Unique to Swoop:** `swoop handoff` (continuation packet), `swoop inbox`
(attention-sorted summary), and `swoop doctor` (structured health check). No
...
| Feature                   | Detail                                                                                                                                                                                                                         |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **swoop sync link**       | Moves a project's session transcripts from `~/.claude/projects/` into `.claude-memory/` inside the project directory, then redirects Claude Code's storage there via a filesystem junction (Windows) or symlink (macOS/Linux). |
| **Cloud sync compatible** | Any cloud folder syncing the project (OneDrive, pCloud, Dropbox, Google Drive) carries the sessions along automatically. No cloud account required in Swoop itself.                                                            |
| **Second-machine setup**  | `swoop sync link <path>` on a new machine computes the expected project ID from the path and creates the redirect automatically if no local entry exists.                                                                      |
| **Reversible**            | `swoop sync unlink` restores local-only storage.                                                                                                                                                                               |
| **Cloud indicator**       | Linked projects show `☁` in TUI and web project rows.                                                                                                                                                                          |

**Unique to Swoop:** no other reviewed tool ships a session portability mechanism
of any kind.
...
| Feature                         | Detail                                                                                                                                                      |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Zero-config defaults**        | Works out of the box with `swoop`. No config file required.                                                                                                 |
| **swoop config CLI**            | `swoop config get/set/reset <key>` reads and writes `~/.claude/swoop/prefs.json`.                                                                           |
| **swoop config TUI**            | Keyboard-navigable Interface, Integrations, and Features tabs. Integrations show their exact effect and are reversible from within the UI.                  |
...
| **No telemetry**                              | Swoop collects no usage metrics, crash reports, or any other data.                                                                                                                     |
| **Localhost web server**                      | The web server binds to `127.0.0.1` only. Not reachable from other machines.                                                                                                           |
| **Safe transcript handling**                  | Automated cleanup only archives through Swoop sidecar metadata. Permanent deletion is an explicit action, is blocked for active sessions, and is never used by background maintenance. |
...
| Capability                                          |     swoop      | Mantra |  CCHV   | claude-history | Built-in |
| --------------------------------------------------- | :------------: | :----: | :-----: | :------------: | :------: |
...
- **Timeline replay / step-by-step code diff** — Mantra's differentiator. Too
  heavy for our light/local mission and does not answer the question Swoop is
...
- **Embedded terminal or Electron desktop app** — deliberately unnecessary.
  Swoop opens in a terminal or browser tab you already have, starts in under a
