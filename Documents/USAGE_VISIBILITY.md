...
This document defines how Swoop presents Claude Code usage without guessing while
...
- **Live plan limits** are account-level facts from a supported Claude Code
  integration.
- Missing data remains unavailable. Swoop never turns an absent value into zero.

## Source Matrix

| Source                                                                | Supported facts                                                             | Trust and limitations                                                                     | Swoop use                                         |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------- |
...
| [OpenTelemetry](https://code.claude.com/docs/en/monitoring-usage)     | Detailed token, model, cost, and agent telemetry                            | Powerful but opt-in and substantially heavier than Swoop's default design                 | Optional future integration only                  |
...
Swoop extracts these facts while it already scans a transcript:
...
When Swoop uses `sessions-index.json` without opening a transcript, all context
metrics are `null`.

## Implemented: Live Account Limits

After explicit setup, Swoop refreshes the same read-only account usage endpoint
...
after Swoop setup, removal refuses to overwrite the newer user configuration.

Swoop reads Claude Code's local OAuth access token only while making the request.
It never logs, returns, or copies the token into Swoop storage. Successful
...
integration. Swoop therefore never claims that a cached value is live merely
...
until a supported source exposes them. Swoop may show a positive `credits on`
...
unknown; Swoop never infers activation from cost or rate-limit data.

## Safety And Privacy

- Read Claude's OAuth access token only after explicit `swoop usage setup`, keep it
...
- `swoop usage remove` deletes captured session snapshots and the aggregate account cache.
