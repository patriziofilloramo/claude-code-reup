...
This document defines how CCM presents Claude Code usage without guessing while
...
- **Live plan limits** are account-level facts from a supported Claude Code
  integration.
- Missing data remains unavailable. CCM never turns an absent value into zero.

## Source Matrix

| Source                                                                | Supported facts                                                             | Trust and limitations                                                                     | CCM use                                           |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------- |
...
| [OpenTelemetry](https://code.claude.com/docs/en/monitoring-usage)     | Detailed token, model, cost, and agent telemetry                            | Powerful but opt-in and substantially heavier than CCM's default design                   | Optional future integration only                  |
...
CCM extracts these facts while it already scans a transcript:
...
When CCM uses `sessions-index.json` without opening a transcript, all context
metrics are `null`.

## Implemented: Live Account Limits

After explicit setup, CCM refreshes the same read-only account usage endpoint
...
after CCM setup, removal refuses to overwrite the newer user configuration.

CCM reads Claude Code's local OAuth access token only while making the request.
It never logs, returns, or copies the token into CCM storage. Successful
...
integration. CCM therefore never claims that a cached value is live merely
...
until a supported source exposes them. CCM may show a positive `credits on`
...
unknown; CCM never infers activation from cost or rate-limit data.

## Safety And Privacy

- Read Claude's OAuth access token only after explicit `ccm usage setup`, keep it
...
- `ccm usage remove` deletes captured session snapshots and the aggregate account cache.
