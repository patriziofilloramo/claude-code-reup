...
CCM should feel native to install while remaining transparent about changes to
the user's machine.
...
The Windows installer should install a self-contained CCM executable or launcher
and add it to the current user's `PATH`.

It should offer a pre-selected option:

> Enable PowerShell tab completion for `ccm resume` and `ccm handoff`

When accepted, the installer should:

1. Install a versioned completion script inside CCM's installation directory.
...
4. Back up an existing profile before its first CCM modification.
...
```powershell
# >>> ccm completion >>>
. "$env:LOCALAPPDATA\Programs\ccm\completion\ccm.ps1"
# <<< ccm completion <<<
```
...
Shell completion remains opt-in through `ccm completion <shell>`. A future
managed `ccm config` interface may install or remove the same integration after
